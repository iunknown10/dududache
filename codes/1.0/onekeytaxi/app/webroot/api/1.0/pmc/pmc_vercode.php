...
if (API_METHOD_GET == $_SERVER['REQUEST_METHOD']) {
    ...
    //判断现有表中是否存在
    $sql = 'select username from '.API_TABLE_PRE.'passenger_vercode where username=\''.$mobilePhone.'\'';
    $rs = myDoSqlQuery($sql);
    $num = pg_num_rows($rs);
    if($num){
    	$sql = 'delete from '.API_TABLE_PRE.'passenger_vercode where username=\''.$mobilePhone.'\'';
    	$rs = myDoSqlQuery($sql);
    }
    $tmpRand = rand(1000,9999);
    $validTime = time()+60*SMS_VERCODE_VALID;//验证码5分钟内有效
    $sql = 'INSERT INTO '.API_TABLE_PRE.'passenger_vercode(' .
